...
Sometimes you will hear people speak of _executing_ a function or _calling_ a function. These are synonyms for running a function.
...
* How would _you_ store this number `2.718282` as `e` in R's memory? 
...
The combination looks like an arrow. `<-` creates a new _object_ that has the name that appears on the left side of the arrow and the value that appears on the right side of the arrow. 
...
1. A name cannot contain a special character like, `+`, `-`, `/`, `*`, `^`, `%`, `!`, `$`, `@`, `'`, `"`. R wouldn't be able to tell if we were naming an object or doing some math. One exception is `_`, which is often used *in* names (but a name can't start with `_`).
...
1. Create an object named `world` that contains the value `44`
1. Add `hello` to `world`
...
**Now add a function to the third line to find the square root of `hello + world`.**  Be sure to remove each `_` before you run your code._
...
R users will usually rewrite nested functions as _chained_ functions, which are easier to read. You can chain two functions together with the pipe symbol, which looks like this, `%>%`.
...
In R, spreadsheet-like tables are called "tibbles" or "data frames." This distinguishes them from matrices and arrays, which are another type of table.
...
R doesn't use every package you have installed every time you open R. R waits for you to tell it which packages you would like to use during _that_ R session. By R session, I mean the time from when you open R (or the RStudio IDE) until when you close it. 
